        ...
        if(wallObjects.Count > 0)
        {
            ...
            for (int j = 0; j < parentRooms[i].Corners.Count; j++)
            {
                ...
                if (tempVec3.x == 1)
                {
                    ...
                    GameObject tempWallObject = Instantiate(wallObjects[randomWallObject].gameObject, new Vector3(area[tempVec3.y, tempVec3.z].transform.position.x + tempTorchDisplacement.x, tempTorchDisplacement.y, area[tempVec3.y, tempVec3.z].transform.position.z + tempTorchDisplacement.z), Quaternion.Euler(-90, extraYAngle, 90), parentRooms[i].transform);
                ...
                if( corridors[i].transform.childCount > wallObjConsequitiveDistance)
                {
                    ...
                    for (int j = 0; j < corridors[i].transform.childCount; j++)
                    {
                        if(j > 0 && corridors[i].transform.GetChild(j).CompareTag("Floor"))
                        {
                            ...
                            {
                                int randomWallObject = Random.Range(0, wallObjects.Count);
                                GameObject tempWallObject = Instantiate(wallObjects[randomWallObject].gameObject, new Vector3(position.x + wallObjDisplacements[randomWallObject].z, wallObjDisplacements[randomWallObject].y, position.z + wallObjDisplacements[randomWallObject].x), Quaternion.Euler(-90, 0, 0), corridors[i].transform);
                                tempWallObject.name = wallObjects[randomWallObject].name;
                                checkIteration = 0;
                            }
                            ...
                            {
                                int randomWallObject = Random.Range(0, wallObjects.Count);
                                GameObject tempWallObject = Instantiate(wallObjects[randomWallObject].gameObject, new Vector3(position.x + wallObjDisplacements[randomWallObject].x, wallObjDisplacements[randomWallObject].y, position.z + wallObjDisplacements[randomWallObject].z), Quaternion.Euler(-90, 0, -90), corridors[i].transform);
                                tempWallObject.name = wallObjects[randomWallObject].name;
                                checkIteration = 0;  
                            }
            ...
            for (int a = 0; a < parentRooms.Length; a++)
            {
                for (int b = 0; b < parentRooms[a].MidPoints.Count; b++)
                {
                    ...
                    if(randomChange > 0 && randomChange < middleObjectsRate)
                    {
                        ...
                        {
                            GameObject tempMiddleObject = Instantiate(middleObjects[randomMiddleObject].gameObject, parentRooms[a].MidPoints[b].transform.position, Quaternion.Euler(-90, randomRotation, 0), parentRooms[a].transform);
        ...
        if(!CheckIfNull(area[coords.x + additive.x, coords.y + additive.y], parentRooms[i].Corners[j]) && area[coords.x + additive.x, coords.y + additive.y].transform.parent != null)
        {
            if (randomNumber > 0 && randomNumber <= cornerObjectsRate)
            {
                if(area[coords.x + additive.x, coords.y + additive.y].transform.parent.CompareTag("Room") && area[coords.x + additive.x, coords.y + additive.y].transform.position != parentRooms[i].Corners[j].transform.position)
                {
                    ...
                    area[coords.x + additive.x, coords.y + additive.y] = Instantiate(tempContent, area[coords.x + additive.x, coords.y + additive.y].transform.position, Quaternion.Euler(-90, 0, -45), parentRooms[i].Corners[j].transform.parent.transform);
